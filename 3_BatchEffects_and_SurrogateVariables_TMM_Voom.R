...
load(file='BatchEffects_and_SurrogateVariables_TMM_Voom.RData')
...
j=1
  ...
  # A lot of PC2 structure still remains! -> induced by cercropins!
...
# Look at PC2 -> there are antennal genes but also non-antennal genes reflecting a high difference
hist(counts.adj5["FBgn0038484",Ctrl_flies])
hist(counts.adj5["FBgn0038484",HS_flies],add=T,col=rgb(0,1,1,0.5))
hist(counts.adj5["FBgn0011293",Ctrl_flies])
hist(counts.adj5["FBgn0011293",HS_flies],add=T,col=rgb(0,1,1,0.5)) # doubly bimodal
plot(counts.adj5["FBgn0011293",],counts.adj["FBgn0038484",])
plot(counts.adj5["FBgn0011293",],counts.adj["FBgn0010403",])


##############################################
##### (3f) Counts - batch effects - 5 SVs ####
##############################################
  
# Correction for batch effects and 5 surrogate variables
covariates6 <- model.matrix(~rnalib+rnaseq+egg+plate+well+sv1.4+sv2.4+sv3.4+sv4.4+sv5.4)
counts.adj6 <- limma::removeBatchEffect(voom.countsm, covariates = covariates6[,-1],  design = nullmodel) 

# View PCs
pca6 <- prcomp(t(counts.adj6))
plot(pca6$x[,1] ~ pca6$x[,2])
pca6.w = NULL
  for (i in 1:length(pca6$sdev)) {
    pca6.w[i]= (pca6$sdev[i])^2/sum(pca6$sdev^2) 
  }
plot(pca6.w[1:50])
  
fiftypcs6 <- pca6$x[, 1:50]
par(mfrow=c(1,1))
  
# Check for residual batch effects
mcounts.adj6 <-lm(pca6$x[,1:200] ~ treatment + rnalib + rnaseq + egg + plate + sv1.4 + sv2.4 + sv3.4 + sv4.4 + sv5.4)
mcounts.adj6_well <-lm(pca6$x[,1:200] ~ treatment + rnalib + rnaseq + egg + plate + sv1.4 + sv2.4 + sv3.4 + sv4.4 + sv5.4 + well)
anova(mcounts.adj6)
anova(mcounts.adj6_well)
anova(mcounts.adj6,mcounts.adj6_well)
  ...
  #plot treatment - 
  plot(fiftypcs6[,i]~ fiftypcs6[,j], col="white", pch=16, main="Treatment")
  points(fiftypcs6[,i][cov2$treatment=="1"]~ fiftypcs6[,j][cov2$treatment=='1'], col="blue", pch=1)
  points(fiftypcs6[,i][cov2$treatment=="2"]~ fiftypcs6[,j][cov2$treatment=='2'],col="red", pch=1)
  # PC2 structure largely gone
  # Clean separation between treatments
  
  #plot sequencing batch
  plot(fiftypcs6[,i] ~ fiftypcs6[,j], col="white", pch=16,  main="Seq batch")
  points(fiftypcs6[,i][cov2$RNAseqBatch==1]~ fiftypcs6[,j][cov2$RNAseqBatch==1], col="blue", pch=1)
  points(fiftypcs6[,i][cov2$RNAseqBatch==2]~ fiftypcs6[,j][cov2$RNAseqBatch==2],col="red", pch=1)
  points(fiftypcs6[,i][cov2$RNAseqBatch==3]~ fiftypcs6[,j][cov2$RNAseqBatch==3],col="green", pch=1)
  points(fiftypcs6[,i][cov2$RNAseqBatch==4]~ fiftypcs6[,j][cov2$RNAseqBatch==4],col="pink", pch=1)
  points(fiftypcs6[,i][cov2$RNAseqBatch==5]~ fiftypcs6[,j][cov2$RNAseqBatch==5],col="grey", pch=1)
  
  
  #plot library prep batch
  plot(fiftypcs6[,i] ~ fiftypcs6[,j], col="white", pch=16,  main="Lib prep batch")
  points(fiftypcs6[,i][cov2$RNAlibBatch==1]~ fiftypcs6[,j][cov2$RNAlibBatch==1], col="blue", pch=1)
  points(fiftypcs6[,i][cov2$RNAlibBatch==2]~ fiftypcs6[,j][cov2$RNAlibBatch==2],col="red", pch=1)
  points(fiftypcs6[,i][cov2$RNAlibBatch==3]~ fiftypcs6[,j][cov2$RNAlibBatch==3],col="black", pch=1)
  
  #plot egglay batch
  plot(fiftypcs6[,i] ~ fiftypcs6[,j], col="white", pch=16,  main="Egglay batch")
  c <- sample(colours(),3)
  for (e in 1){
    points(fiftypcs6[,i][cov2$egglayBatch==e]~ fiftypcs6[,j][cov2$egglayBatch==e], col=c[e], pch=16)
  }
  for (e in 2){
    points(fiftypcs6[,i][cov2$egglayBatch==e]~ fiftypcs6[,j][cov2$egglayBatch==e], col=c[e], pch=16)
  }
  ...
  #plot plating day batch
  plot(fiftypcs6[,i] ~ fiftypcs6[,j], col="white", pch=16,  main="Plating day batch")
  c <- sample(colours(),7)
  ee <- c(21:27)
  points(fiftypcs6[,i][cov2$platingBatch==ee[1]]~ fiftypcs6[,j][cov2$platingBatch==ee[1]], col=c[1], pch=1)
  points(fiftypcs6[,i][cov2$platingBatch==ee[2]]~ fiftypcs6[,j][cov2$platingBatch==ee[2]], col=c[2], pch=1)
  points(fiftypcs6[,i][cov2$platingBatch==ee[3]]~ fiftypcs6[,j][cov2$platingBatch==ee[3]], col=c[3], pch=1)
  points(fiftypcs6[,i][cov2$platingBatch==ee[4]]~ fiftypcs6[,j][cov2$platingBatch==ee[4]], col=c[4], pch=1)
  points(fiftypcs6[,i][cov2$platingBatch==ee[5]]~ fiftypcs6[,j][cov2$platingBatch==ee[5]], col=c[5], pch=1)
  points(fiftypcs6[,i][cov2$platingBatch==ee[6]]~ fiftypcs6[,j][cov2$platingBatch==ee[6]], col=c[6], pch=1)
  points(fiftypcs6[,i][cov2$platingBatch==ee[7]]~ fiftypcs6[,j][cov2$platingBatch==ee[7]], col=c[7], pch=1)
...
# The well models are always significantly better than those without
...
# PCA plot of the expression matrix with 4 surrogate variables regressed out
dataframe1 <- data.frame(ID = rownames(fiftypcs5), EV1 = fiftypcs5[,'PC1'],EV2 = fiftypcs5[,'PC2'])
dataframe2 <- data.frame(ID = rownames(cov2), Treatment = cov2[,'treatment'])
PCplot_df <- merge(dataframe1,dataframe2,by='ID',all.x=T)
PCplot_df$Diet <- ifelse(PCplot_df$Treatment==1,'Control','High Sugar')

PCplot <- ggplot(data=PCplot_df,aes(x=EV1,y=EV2,group=Diet,col=Diet))+geom_point()+ 
  scale_color_manual(values=c('#C6B49F','#DF9F65')) + 
  geom_point() + 
  ylab(paste0('Expression PC2 (',100*round(pca5.w[2],3),'%)')) + 
  xlab(paste0('Expression PC1 (',100*round(pca5.w[1],3),'%)')) +
...
setwd("C:\\Users\\jtanshengyi\\Desktop\\Projects\\veQTL Netherlands Normal vs High Sugar Adult\\Data\\")
ggsave(filename = 'Transcriptome PCA.png',plot = PCplot,height = 3,width = 3)
ggsave(filename = 'Transcriptome PCA nolegend.png',plot = PCplot_nolegend,height = 3,width = 3)
...
write.table(sv1.4, 'TMM_Voom_sv1_10.txt')
write.table(sv2.4, 'TMM_Voom_sv2_10.txt')
write.table(sv3.4, 'TMM_Voom_sv3_10.txt')
write.table(sv4.4, 'TMM_Voom_sv4_10.txt')

# Include the sample IDs - for the subsampling analyses
write.table(data.frame(fly_id=colnames(voom.countsm),x=sv1.4), 'TMM_Voom_sv1_10_wID.txt',row.names = F)
write.table(data.frame(fly_id=colnames(voom.countsm),x=sv2.4), 'TMM_Voom_sv2_10_wID.txt',row.names = F)
write.table(data.frame(fly_id=colnames(voom.countsm),x=sv3.4), 'TMM_Voom_sv3_10_wID.txt',row.names = F)
write.table(data.frame(fly_id=colnames(voom.countsm),x=sv4.4), 'TMM_Voom_sv4_10_wID.txt',row.names = F)

save.image(file='NvsHS_BatchEffects_and_SurrogateVariables_TMM_Voom.RData')
