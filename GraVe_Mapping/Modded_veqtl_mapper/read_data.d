...
struct Phenotype
{
  ...
  this(const char[] line, const size_t[] indices)
  {
    ...
    startpos = splitLine[1].to!size_t;
    endpos = splitLine[2].to!size_t;
...
auto readGenotype(const Opts opts, string chrom, size_t startpos, size_t endpos, string geneName)
{

  immutable size_t start = startpos < opts.window ? 0 : startpos - opts.window;

  string tabixCommand = "tabix " ~ opts.vcf ~ " " ~ chrom ~ ":" ~ start.to!string ~ "-" ~ (
      endpos + opts.window).to!string ~ " | grep -v '#' | cut -f1,2,4,5,10-";
