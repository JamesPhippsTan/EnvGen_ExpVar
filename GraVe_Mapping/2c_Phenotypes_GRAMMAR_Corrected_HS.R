# Applying the GRAMMAR correction - removing GRM from the data - HS dataset
...
GP_Data <- load.gwaa.data(genofile = 'Dmel_HS_MAF5_Miss50_GRAMMAR.raw',phenofile = 'GraVe_phenotypes_HS.txt',id='id')

# GRM to estimate unknown relatedness - all individuals, all SNPs
GRM <- ibs(GP_Data, w="freq")

# Retrieve the subset of phenotypes to be transformed (3 because the phenotypes start from column 3)
...
first_gene <- start-2
last_gene <- end-2
write.table(P_Data_GRAMMARed,paste0('GRAMMARed_phenotypes_HS_',first_gene,'_',last_gene,'.txt'),quote=F)
