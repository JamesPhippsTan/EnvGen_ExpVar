...
def calculate_cis_permutations(genotypes_t, phenotype_t, permutation_ix_t,
    ...
    # Change 2 - Return results of all SNPs
    return r_nominal_t, std_ratio_t, torch.arange(genotypes_t.shape[0]), r2_perm_t, r2_perm_t_all, genotypes_t
    # Change 2 - Return results of all SNPs
...
def prepare_cis_output(r_nominal, r2_perm, r2_perm_max, std_ratio, g, num_var, dof, variant_id, start_distance, end_distance, phenotype_id, nperm=10000):
    ...
    res_s = pd.Series(OrderedDict([
        ...
        ('true_df', np.NaN),
        ('pval_true_df', np.NaN),
        ('beta_shape1_max', np.NaN),
        ('beta_shape2_max', np.NaN),
        ('true_df_max', np.NaN),
        ('pval_true_df_max', np.NaN),
        ...
        ('slope_se', slope_se),
        ('pval_perm', pval_perm),
        ('pval_perm_max', pval_perm_max),
        ('pval_beta', np.NaN),
        ('pval_beta_max', np.NaN),
    ]), name=phenotype_id)
    return res_s
...
# Change 3 - res is now a list of SNPs for a single gene rather than a single gene-SNP pair and saves this into a single file per gene
            ...
            for snp_index in var_ix: # Prepare output for each SNP
                var_index = genotype_range[snp_index]
                variant_id = variant_df.index[var_index]
                start_distance = variant_df['pos'].values[var_index] - igc.phenotype_start[phenotype_id]
                end_distance = variant_df['pos'].values[var_index] - igc.phenotype_end[phenotype_id]
                res_s = prepare_cis_output(r_nominal[snp_index],r2_perm_t_all[snp_index].flatten(),r2_perm,std_ratio[snp_index],g[snp_index],genotypes_t.shape[0],idof,variant_id,start_distance,end_distance,phenotype_id,nperm=nperm)
                if beta_approx:
