...
#load(file='4b_Defining_GxE_eQTL_FDR1.RData')
...
# While we can grab the nominal p-values from the original files, the original BH-corrected p-values are absent
...
row_labels <- c("cis_FDR<0.1", "cis_FDR<0.05", "cis_FDR<0.01",
                "trans_FDR<0.1", "trans_FDR<0.05", "trans_FDR<0.01"
                , "cisandtrans_FDR<0.1", "cisandtrans_FDR<0.05", "cisandtrans_FDR<0.01")
...
write.csv(eQTL_GxE_table,'eQTL_GxE_table_FDR1.csv',row.names = F)
...
for (n_pval in 0:2){
  firstrow <- (n_pval*3)+1
  lastrow <- firstrow + 2
  eQTL_GxE_table_percent[firstrow:lastrow,2:4] <- eQTL_GxE_table_percent[firstrow:lastrow,2:4]*100/(sum(eQTL_GxE_table_percent[firstrow,2:4]))
  eQTL_GxE_table_percent[firstrow:lastrow,5:7] <- eQTL_GxE_table_percent[firstrow:lastrow,5:7]*100/(sum(eQTL_GxE_table_percent[firstrow,5:7]))
}
eQTL_GxE_table_percent[,2:7] <- round(eQTL_GxE_table_percent[,2:7],digits = 2)
View(eQTL_GxE_table_percent)
setwd("C:\\Users\\jtanshengyi\\Desktop\\Projects\\veQTL Netherlands Normal vs High Sugar Adult\\Data\\GraVe_Mapping\\GxE")
write.csv(eQTL_GxE_table_percent,'eQTL_GxE_table_percent_FDR1.csv')
...
eQTL_GxE_table_percent_separated$shared_cutoff <- factor(
  ...
  levels = c("FDR<0.01", "FDR<0.05", "FDR<0.1")
)
...
for (col_name in columns_to_plot) {
  ...
  # Save plot
  ggsave(filename = paste0(col_name, "_eQTL_dotplot_FDR1.svg"),
...
save.image(file='4b_Defining_GxE_eQTL_FDR1.RData')
